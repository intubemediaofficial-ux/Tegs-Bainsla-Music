...
function itemClasses(active: boolean) {
  return `flex items-center gap-3 rounded-xl px-3 py-2.5 text-[15px] font-semibold transition ${
    active
      ? "bg-brand-600/20 text-brand-100 ring-1 ring-brand-500/50"
      : "text-slate-300 hover:bg-ink-card hover:text-white"
  }`;
}
...
export function AppSidebar({
  ...
  return (
    <aside className="hidden w-64 shrink-0 flex-col border-r border-ink-line bg-ink-soft p-4 md:flex">
      ...
      <div className="mt-4 rounded-xl border border-ink-line bg-ink-card p-3 text-xs">
...
export function AppMobileNav({ name, isAdmin }: { name: string; isAdmin: boolean }) {
  ...
  return (
    <div className="md:hidden">
      <header className="sticky top-0 z-30 flex items-center justify-between gap-3 border-b border-ink-line bg-ink/95 px-4 py-3 backdrop-blur">
        <button
          type="button"
      ...
      {open && (
        <div className="fixed inset-0 z-40 bg-black/60" onClick={() => setOpen(false)}>
          <div
            className="h-full w-[80%] max-w-xs overflow-y-auto border-r border-ink-line bg-ink-soft p-4"
